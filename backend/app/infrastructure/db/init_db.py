...
async def _ensure_view_token_secret_column(conn_engine: AsyncEngine) -> None:
    """Add view_token_secret for order confirmation email deep links."""
    settings = get_settings()
    url = settings.database_url
    async with conn_engine.begin() as conn:
        if "sqlite" in url:
            result = await conn.execute(text("PRAGMA table_info(orders)"))
            columns = {row[1] for row in result.fetchall()}
            if "view_token_secret" not in columns:
                await conn.execute(
                    text(
                        "ALTER TABLE orders ADD COLUMN view_token_secret VARCHAR(256)"
                    )
                )
            return
        ...
        check = await conn.execute(
            text(
                ...
                WHERE table_schema = 'public'
                  AND table_name = 'orders'
                  AND column_name = 'view_token_secret'
                """
            )
        )
        if check.fetchone() is None:
...
async def init_db() -> None:
    ...
    await _ensure_view_token_secret_column(engine)
